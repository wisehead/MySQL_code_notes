...
# variable for gaia
my $opt_gaia_manager_basedir;
my $opt_gaia_log_service_basedir;
my $opt_gaia_page_server_basedir;
my $opt_gaia_skip_init;
my $opt_start_port = 0;
my $opt_local_host;
...
sub command_line_setup {
  ...
  my %options = (
    ...
    # gaia directories
    'gaia-manager-basedir=s' => \$opt_gaia_manager_basedir,
    'gaia-log-service-basedir=s' => \$opt_gaia_log_service_basedir,
    'gaia-page-server-basedir=s' => \$opt_gaia_page_server_basedir,
    'gaia-skip-init' => \$opt_gaia_skip_init,
    'local-host=s' => \$opt_local_host,
    'start-port=i' => \$opt_start_port,
...
sub mysql_install_db {
  ...
  mtr_add_arg($args, "--innodb_buffer_pool_size=1024M");
  mtr_add_arg($args, "--innodb-log-file-size=5M");
  mtr_add_arg($args, "--manager_address=http://$opt_local_host:$opt_start_port");
  mtr_add_arg($args, "--innodb_rpc_server_port=%d", $opt_start_port+3);
...
sub clean_datadir {
  ...
  foreach my $mysqld (mysqlds()) {
    ...
    $mysqld->{'ps_proc'} = undef;
    mtr_verbose("page server stopped");
    sleep 3;
    My::SafeProcess::shutdown(0, $mysqld->{'ls_proc'});
    $mysqld->{'ls_proc'} = undef;
    mtr_verbose("log service stopped");
    sleep 3;
    My::SafeProcess::shutdown(0, $mysqld->{'psm_proc'});
    $mysqld->{'psm_proc'} = undef;
    mtr_verbose("page server manager stopped");
    sleep 3; 
    ...
    if (-d $mysqld_dir and
        ...
        !$bootstrap_opts) {
      my $group = $mysqld->value('#fix_dir');
      mtr_verbose(" - removing $group/ls_data");
      rmtree("$group/ls_data");
      sleep 1;
      mtr_verbose(" - removing $group/ps_data");
      rmtree("$group/ps_data");
      sleep 1;
      mtr_verbose(" - removing $group/psm_data");
...
sub registe_service($$$$$) {
  ...
  system("curl -d '{\"addr\":\"$host:$ls_port\"}' http://$host:$psm_port/ManageService/add_log_service");
  sleep 1;
  system("curl -d '{\"segments\":[{\"replica_addrs\":[\"$host:$ps_port\",\"$host:19009\"]}]}' http://$host:$psm_port/ManageService/add_segment");
  sleep 1;
  system("curl -d '{\"segment_id\":1, \"replica_id\":1}' http://$host:$psm_port/ManageService/drop_replica");
  sleep 1;
  system("curl -d '{\"server_id\":\"1\",\"addr\":\"$host:$pr_port\",\"is_primary\":true}' http://$host:$psm_port/ManageService/add_compute_node");
  sleep 1;
}

sub gaia_manager_start($$$$) {
  my $exec = shift;
  my $host = shift;
  my $port = shift;
  my $data_dir = shift;

  mtr_report("Start gaia manager");

  my $args;
  mtr_init_args(\$args);
  mtr_add_arg($args, "--manager_rpc_port=%s", $port);
  mtr_add_arg($args, "--bthread_concurrency=%d", 18);
  mtr_add_arg($args, "--crash_on_fatal_log=%s", "true");
  mtr_add_arg($args, "--manager_raft_initial_conf=%s:%d:0", $host, $port);

  mtr_add_arg($args, "--manager_raft_dir=%s", "local://$data_dir");
  mtr_add_arg($args, "--comlog_path=%s", "$data_dir/log");
  mtr_add_arg($args, "--lock_file_path=%s", "$data_dir");
  mtr_add_arg($args, "--pid_file_path=%s", "$data_dir");

  my $path_gaia_manager_bootstrap_log = "$opt_vardir/log/psm_bootstrap.log";

  my $proc = My::SafeProcess->new(name      => "gaia_manager",
                                  path      => $exec,
                                  error     => $path_gaia_manager_bootstrap_log,
                                  output    => $path_gaia_manager_bootstrap_log,
                                  args      => \$args,
                                  user_data => $path_gaia_manager_bootstrap_log,
                                  verbose   => $opt_verbose,);
...
sub gaia_log_service_start($$$$$) {
  my $exec = shift;
  my $host = shift;
  my $port = shift;
  my $manager_port = shift;
  my $data_dir = shift;

  mtr_report("Start gaia log service");

  my $args;
  mtr_init_args(\$args);
  mtr_add_arg($args, "--lsrv_rpc_port=%d", $port);
  mtr_add_arg($args, "--manager_address=%s:%d", $host, $manager_port);
  mtr_add_arg($args, "--lsrv_raft_initial_conf=%s:%d:0", $host, $port);

  mtr_add_arg($args, "--enable_afs=%s", "false");
  mtr_add_arg($args, "--lsrv_afs_uri=afs://gaia-stable.afs.baidu-int.com:8400");
  mtr_add_arg($args, "--lsrv_afs_username=qa");
  mtr_add_arg($args, "--cluster_id=1");
  mtr_add_arg($args, "--lsrv_afs_password=qa");
  mtr_add_arg($args, "--afs_worker_threads=%d", 4);
  mtr_add_arg($args, "--afs_redolog_dir=/user/qa/gaiadb/gaiadbenv/primary/var/");
  mtr_add_arg($args, "--afs_archive_dir=/user/qa/");
  mtr_add_arg($args, "--afs_redolog_basename=ib_logfile0");
  mtr_add_arg($args, "--local_addr=0.0.0.0:8888");
  mtr_add_arg($args, "--ns_call_timeout_ms=30000");
  mtr_add_arg($args, "--ns_connect_timeout_ms=3000");
  mtr_add_arg($args, "--ns_retry_interval_ms=1500");
  mtr_add_arg($args, "--ns_max_retry_interval_ms=10000");
  mtr_add_arg($args, "--ms_call_timeout_ms=30000");
  mtr_add_arg($args, "--ms_connect_timeout_ms=3000");
  mtr_add_arg($args, "--ms_retry_interval_ms=1500");
  mtr_add_arg($args, "--ms_max_retry_interval_ms=10000");
  mtr_add_arg($args, "--dn_call_timeout_ms=30000");
  mtr_add_arg($args, "--dn_connect_timeout_ms=3000");
  mtr_add_arg($args, "--dn_retry_interval_ms=1500");
  mtr_add_arg($args, "--dn_max_retry_interval_ms=10000");
  mtr_add_arg($args, "--afs_use_rdma=false");

  mtr_add_arg($args, "--comlog_path=%s", "$data_dir/log");
  mtr_add_arg($args, "--lsrv_comlog_path=%s", "$data_dir/log");
  mtr_add_arg($args, "--lock_file_path=%s", "$data_dir");
  mtr_add_arg($args, "--pid_file_path=%s", "$data_dir");

  my $path_gaia_log_service_bootstrap_log = "$opt_vardir/log/ls_bootstrap.log";

  my $proc = My::SafeProcess->new(name      => "gaia_log_service",
                                  path      => $exec,
                                  error     => $path_gaia_log_service_bootstrap_log,
                                  output    => $path_gaia_log_service_bootstrap_log,
                                  args      => \$args,
                                  user_data => $path_gaia_log_service_bootstrap_log,
                                  verbose   => $opt_verbose,);
...
sub gaia_page_server_start($$$$$$) {
  ...
  mtr_report("Start gaia page server");

  my $args;
  mtr_init_args(\$args);

  mtr_add_arg($args, "--no-defaults");
  mtr_add_arg($args, "--innodb_reserve_page_store=%s", $new_install);
  mtr_add_arg($args, "--innodb_rbio_server_port=%d", $port);
  mtr_add_arg($args, "--manager_address=%s:%d", $host, $manager_port);
  mtr_add_arg($args, "--innodb_page_store_dir=/user/qa/psvr_page_store/");
  mtr_add_arg($args, "--innodb_checkpoint_dir=/user/qa/psvr_page_store/");
  mtr_add_arg($args, "--innodb_page_store_file_size=8192M");
  mtr_add_arg($args, "--innodb_checkpoint_file_size=64K");
  mtr_add_arg($args, "--innodb_afs_host=afs://gaia.afs.baidu.com:8400");
  mtr_add_arg($args, "--innodb_afs_user=qa");
  mtr_add_arg($args, "--innodb_afs_password=qa");
  mtr_add_arg($args, "--innodb_afs_file_path=/user/qa");
  mtr_add_arg($args, "--innodb_prpl_max_pull_size=256K");
  mtr_add_arg($args, "--innodb_rbio_worker_threads=4");
  mtr_add_arg($args, "--innodb_rbio_dispatch_threhold=2");
  mtr_add_arg($args, "--innodb_rbio_wq_wait=5000000");
  mtr_add_arg($args, "--innodb_file_per_table");
  mtr_add_arg($args, "--innodb_buffer_pool_size=1G");
  mtr_add_arg($args, "--innodb_read_io_threads=20");
  mtr_add_arg($args, "--innodb_write_io_threads=20");
  mtr_add_arg($args, "--innodb_thread_concurrency=16");
  mtr_add_arg($args, "--innodb_use_native_aio=0");
  mtr_add_arg($args, "--innodb_flush_log_at_trx_commit=1");
  mtr_add_arg($args, "--innodb_log_buffer_size=8M");
  mtr_add_arg($args, "--innodb_log_file_size=128M");
  mtr_add_arg($args, "--innodb_log_files_in_group=2");

  mtr_add_arg($args, "--pid-file=%s", "$data_dir/mysql.pid");
  mtr_add_arg($args, "--datadir=%s", "$data_dir/data");
  mtr_add_arg($args, "--tmpdir=%s", "$data_dir/tmp");
  mtr_add_arg($args, "--log-error=%s", "$data_dir/mysql.err");

  my $path_gaia_page_server_bootstrap_log = "$opt_vardir/log/ps_bootstrap.log";

  my $proc = My::SafeProcess->new(name      => "gaia_page_server",
                                  path      => $exec,
                                  error     => $path_gaia_page_server_bootstrap_log,
                                  output    => $path_gaia_page_server_bootstrap_log,
                                  args      => \$args,
                                  user_data => $path_gaia_page_server_bootstrap_log,
                                  verbose   => $opt_verbose,);
  ...
  foreach my $mysqld (mysqlds()) {
    ...
    if ($basedir eq $mysqld_basedir) {
      ...
      if (!$opt_start_dirty) {
        ...
        my $install_db = "$path/gaia_data/data";
        if (!-d $datadir or
            (!$bootstrap_opts and
          ...
          sleep 1;
          mtr_verbose(" - copying $group/ls_data");
          copytree("$path/gaia_data/ls_data", "$group/ls_data") if -d $install_db;
          sleep 1;
          mtr_verbose(" - copying $group/ps_data");
          copytree("$path/gaia_data/ps_data", "$group/ps_data") if -d $install_db;
          sleep 1;
          mtr_verbose(" - copying $group/psm_data");
          copytree("$path/gaia_data/psm_data", "$group/psm_data") if -d $install_db;
          sleep 3;
          $mysqld->{'psm_proc'} = gaia_manager_start("$opt_gaia_manager_basedir/bin/manager", $opt_local_host, $opt_start_port, "$group/psm_data");
          sleep 3; 
          $mysqld->{'ls_proc'} = gaia_log_service_start("$opt_gaia_log_service_basedir/bin/log_service", $opt_local_host, $opt_start_port+1, $opt_start_port, "$group/ls_data");
          sleep 3; 
          $mysqld->{'ps_proc'} = gaia_page_server_start("$opt_gaia_page_server_basedir/libexec/mysqld", $opt_local_host, $opt_start_port+2, $opt_start_port, "$group/ps_data", "true");
          sleep 10;
        ...
        if ($ENV{'MYSQLD_BOOTSTRAP_CMD'} && ($initial_bootstrap_cmd ne $ENV{'MYSQLD_BOOTSTRAP_CMD'})) {
    ...
    if ($mysqld->{need_reinitialization}) {
      mtr_verbose("start reinitialization");
      my $group = $mysqld->value('#fix_dir');
      if ($mysqld->{'ps_proc'}) {
        My::SafeProcess::shutdown(0, $mysqld->{'ps_proc'});
        mtr_verbose("shutdown ps");
        $mysqld->{'ps_proc'} = undef;
        sleep 3;
      }
      if ($mysqld->{'ls_proc'}) {
        My::SafeProcess::shutdown(0, $mysqld->{'ls_proc'});
        mtr_verbose("shutdown ls");
        $mysqld->{'ls_proc'} = undef;
        sleep 3;
      }
      ...
      if ($mysqld->{'psm_proc'}) {
        My::SafeProcess::shutdown(0, $mysqld->{'psm_proc'});
        mtr_verbose("shutdown psm");
        $mysqld->{'psm_proc'} = undef;
        sleep 3;
      }


      mtr_verbose("start rm $group/ls_data");
      rmtree("$group/ls_data");
      mtr_verbose("stop rm $group/ls_data");
      sleep 1;
      mtr_verbose("start rm $group/ps_data");
      rmtree("$group/ps_data");
      mtr_verbose("stop rm $group/ps_data");
      sleep 1;
      mtr_verbose("start rm $group/psm_data");
      rmtree("$group/psm_data");
      mtr_verbose("stop rm $group/psm_data");
      sleep 1;
      mtr_verbose("start rm $datadir");
      rmtree($datadir);
      mtr_verbose("stop rm $datadir");
      sleep 1;
      mkpath("$group/psm_data");
      mkpath("$group/ls_data");
      mkpath("$group/ps_data");
...
Options to control directories to use
  ...
  gaia-manager-basedir = PATH
                        The directory where gaia-manager installed.
  gaia-log-service-basedir = PATH
                        The directory where gaia-log-service installed.
  gaia-page-server-basedir = PATH
                        The directory where gaia-page-server installed.
